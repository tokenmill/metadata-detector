...
(deftest core-test
  ...
  (testing "Platts"
    (is (= {:title "Chicago gasoline prices fall to all-time low on abundant supply"
  ...
  (testing "qz.com"
    (is (= {:title "A CEO’s playbook for surviving in the Age of Trump"
            :date  "December 12, 2016"}
           (get-byline "en/qz" "http://qz.com/859311/a-ceos-playbook-for-surviving-in-the-age-of-trump/"))))

  (testing "goaleurope.com"
    (is (= {:title "Ukrainian e-sports analytics startup Mobalytics snags $2.6M seed"
            :date  "2016-11-17T12:19:12+00:00"}
           (get-byline "en/goaleurope" "http://goaleurope.com/2016/11/17/ukrainian-e-sports-analytics-startup-mobalytics-snags-2-6m-seed/"))))

  (testing "blog-techtarget.com"
    (is (= {:title "Datrium snares $55 million, nearly year after DVX release"
            :date  "06 Dec 2016"}
           (get-byline "en/blog-techtarget" "http://searchstorage.techtarget.com/blog/Storage-Soup/Datrium-snares-55-million-nearly-year-after-DVX-release"))))

  (testing "news-techtarget.com"
    (is (= {:title "NexentaEdge DevOps goes gratis for Docker persistent storage"
            :date  "12/13/2016"}
           (get-byline "en/news-techtarget" "http://searchstorage.techtarget.com/news/450404544/NexentaEdge-DevOps-goes-gratis-for-Docker-persistent-storage"))))
  ...
  (testing "searchstorage.techtarget.com"
    (is (= {:title "Clustered NAS: How to choose the right clustered NAS system"
            :date  "August 2010"}
           (get-byline "en/techtarget" "http://searchstorage.techtarget.com/tip/Clustered-NAS-How-to-choose-the-right-clustered-NAS-system")))))
